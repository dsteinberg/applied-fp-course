...
mkResponse
  ...
  -> Response
mkResponse s t lbs =
    responseLBS s [("Content-Type", renderContentType t)] lbs
...
mkAddRequest t lbs =
  let
    t' = mkTopic t -- Either Error Topic
    c' = mkCommentText $ lazyByteStringToStrictText lbs -- Either Error CommentText
  in
    AddRq <$> t' <*> c'
  where
...
mkErrorResponse BadRequest = resp400 PlainText "Unknown Request Type"
...
mkRequest rq =
  case (pathInfo rq, requestMethod rq) of 
    ([topic, "add" ], "POST") -> mkAddRequest topic <$> strictRequestBody rq 
...
handleRequest (AddRq _ _) = Right $ resp200 PlainText "Got Add Request"
handleRequest (ViewRq _) = Right $ resp200 PlainText "Got View Request"
handleRequest ListRq = Right $ resp200 PlainText "Got List Request"
...
app req callback = do
  ...
  -- Attempt 1
  -- let {a = case rqtype of
  --   Left e -> mkErrorResponse e
  --   Right r -> case handleRequest r of 
  --     Left he -> mkErrorResponse he
  --     Right hr -> hr
  --     };
  -- callback a

  -- Attempt 2
  -- let {a = case (handleRequest =<< rqtype) of 
  --   Left e -> mkErrorResponse e
  --   Right r -> r
  --   };
  -- callback a
  ...
  -- Attempt 3
  callback $ either mkErrorResponse id (handleRequest =<< rqtype)
